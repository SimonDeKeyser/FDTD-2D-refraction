...
class FDTD():
    ...
    """
    def __init__(self, dx, kd, dt, nt, source, kdmin, kdmax ,obj,animation=False, fine = False):
    ...
    def fine_init(self):
        self.nxf = int(2*self.nd*3)
        self.nyf = int(self.nd*3)
        self.yf = self.npml + 2*self.nd
        self.xf = 2*self.nd
        self.dxf = self.dx/3
        self.dyf = self.dy/3

        self.oxf = np.zeros((self.nxf + 1, self.nyf))
        self.oyf = np.zeros((self.nxf, self.nyf + 1))
        self.pf = np.zeros((self.nxf, self.nyf))
        return self
    ...
    def timestep_fine(self):
        ...
        self.oxf[::3,1] += self.ox[:self.xf+1,self.yf]
        self.oyf[::3,0] += self.oy[:self.xf,self.yf]
        self.oxf_x = (self.oxf[1:, :] - self.oxf[:-1, :]) / self.dxf
        self.oyf_y = (self.oyf[:, 1:] - self.oyf[:, :-1]) / self.dyf
        self.pf -=(self.c ** 2) * self.dt * (self.oxf_x + self.oyf_y)
        self.oxf[1:-1,:] -= (self.dt/self.dxf) * (self.pf[1:,:]-self.pf[:-1,:])
        self.oyf[:,1:-1] -= (self.dt/self.dxf) * (self.pf[:,1:]-self.pf[:,:-1])
        self.oxf_x = (self.oxf[1:, :] - self.oxf[:-1, :]) / self.dxf
        self.oyf_y = (self.oyf[:, 1:] - self.oyf[:, :-1]) / self.dyf
        self.pf -=(self.c ** 2) * self.dt * (self.oxf_x + self.oyf_y)
        ...
        #self.hard_walls()
        self.p -= (self.c ** 2) * self.dt * (self.ox_x + self.oy_y)
        self.p[:self.xf,self.yf] = self.pf[::3,1]
        return self
    ...
    def FDTD_ana_comparison(self,TF_sim,TF_ana,rel=False):
        k_vec, _, _ = self.k_vec()
        Amplratio = np.abs(TF_sim/TF_ana)
        Amplratio =  1+((Amplratio-1)/self.kd)
        Phasediff = ((np.angle(TF_sim)) - (np.angle(TF_ana)))/self.kd
