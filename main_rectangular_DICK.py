...
def step_SIT_SIP(nx, ny, c, dx, dy, dt):
    ...
    p_y[:, -1] = 0
    p_x[:, -1] = 0
    p_x[:int(2 * nd)+1, int(2 * nd)+1: int(3 * nd)+2] = 0 #Thin sheet
    p_y[:int(2 * nd)+1, int(2 * nd)+1: int(3 * nd)+2] = 0 #Thin sheet
...
d = 1  # lengte d
k = kd / d  # wavenumber
L = 8 * d  # length of simulation domain
...
y_recorder1 = y_bron + 3 * nd  # plaats recorder 1 - location receiver 1
...
for it in range(0, nt):
    ...
    ox[:int(2 * nd), int(2 * nd):int(3 * nd)] = 0  # rectangle sheet
    oy[:int(2 * nd), int(2 * nd):int(3 * nd)] = 0  # rectangle sheet
    ...
    artists = [
        ...
        ax.plot(np.full(np.arange(int(2 * nd)).shape, int(3 * nd)), np.arange(int(2 * nd)), color='k')[0],
    ]
