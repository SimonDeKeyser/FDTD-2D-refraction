...
class FDTD():
    ...
    def params_init(self):
        ...
        self.Axdown = [(i % 4)*self.dx / 4 for i in range(2 * self.nd)]
        self.Axup = [(i + self.dx / 4) for i in self.Axdown]
        self.Ay = [self.dx]*len(self.Axdown)

        self.V = [(self.dx**2 / 8 + i*self.dx) for i in self.Axdown]
    ...
    def PML_init(self):
        ...
        self.kap_y_o = np.append(self.kap_y[:, :self.ny // 2], self.kap_y[:, 1 + self.ny // 2:],
                                 1)  # Kappa for o_y, removing a middle column because the shape is different
        return self
    ...
    def timestep(self):
        ...
        self.ox[1:-1, :] = ((1 - self.kap_x[1:, :] * self.dt / 2) / (1 + self.kap_x[1:, :] * self.dt / 2)) * self.ox[
                                                                                                             1:-1,
                                                                                                             :] - (
                                       self.dt / (self.dx * (1 + self.kap_x[1:, :] * self.dt / 2))) * (
                                       self.p[1:, :] - self.p[:-1, :])
        self.oy[:, 1:-1] = ((1 - self.kap_y_o * self.dt / 2) / (1 + self.kap_y_o * self.dt / 2)) * self.oy[:, 1:-1] - (
                    self.dt / (self.dy * (1 + self.kap_y_o * self.dt / 2))) * (self.p[:, 1:] - self.p[:, :-1])
    ...
    def load(self):
        try:
            recorders = np.load('Saved_simulations/{}_recorders_kd={}.npy'.format(self.obj,
                                                                                  self.kd))  # loading saved recorders, MAKE SURE YOU USED THE SAME PARAMETERS
    ...
    def TF_thin(self, recorder_number):
        ...
        theta_R_m = 2 * np.pi - np.arctan([2 / 5, 4 / 5, 6 / 5])[
            recorder_number - 1]  # angle between recorder and mirror sheet
        theta_S, theta_S_m = (
        np.arctan(10 / 19), np.arctan(10 / 21))  # angle between source and sheet and mirror source
        a, a_m = (np.sqrt(1.9 ** 2 + 1) * self.d,
                  np.sqrt(2.1 ** 2 + 1) * self.d)  # distance from source to top of sheet and mirror source
        b = np.array([np.sqrt(1.5 ** 2 + 1) * self.d, np.sqrt(1.5 ** 2 + 4) * self.d, np.sqrt(1.5 ** 2 + 9) * self.d])[
            recorder_number - 1]  # distance from recorder to top of sheet
        b_m = \
        np.array([np.sqrt(2.5 ** 2 + 1) * self.d, np.sqrt(2.5 ** 2 + 4) * self.d, np.sqrt(2.5 ** 2 + 9) * self.d])[
            recorder_number - 1]  # distance from recorder to top of mirror sheet
        ...
        D_m_down = self.Diffraction(k_vec, theta_S_m, theta_R_m, a_m,
                                    b_m)  # source reflection diffraction at mirror wedge
    ...
    def TF_thick(self, recorder_number):
        self.n = 1.5
        k_vec, _, _ = self.k_vec()
        theta_R = 2 * np.pi - np.pi / 2 - np.arctan([2 / 3, 4 / 3, 2])[
            recorder_number - 1]  # angle between recorder and sheet
        theta_R_m = 2 * np.pi - np.pi / 2 - np.arctan([2 / 5, 4 / 5, 6 / 5])[
            recorder_number - 1]  # angle between recorder and mirror sheet
        theta_S, theta_S_m = (
        np.arctan(10 / 19), np.arctan(10 / 21))  # angle between source and sheet and mirror source
        a, a_m = (np.sqrt(1.9 ** 2 + 1) * self.d,
        ...
        np.array([np.sqrt(2.5 ** 2 + 1) * self.d, np.sqrt(2.5 ** 2 + 4) * self.d, np.sqrt(2.5 ** 2 + 9) * self.d])[
            recorder_number - 1]  # distance from recorder to top of mirror sheet
        D_up = self.Diffraction(k_vec, theta_S, self.n * np.pi, a, self.d) * self.Diffraction(k_vec, 0, theta_R, self.d,
                                                                                              b,
                                                                                              order=2)  # source diffraction
        D_m_up = self.Diffraction(k_vec, theta_S_m, self.n * np.pi, a_m, self.d) * self.Diffraction(k_vec, 0, theta_R,
                                                                                                    self.d, b,
                                                                                                    order=2)  # source reflection diffraction
        D_down = self.Diffraction(k_vec, theta_S, self.n * np.pi, a, self.d) * self.Diffraction(k_vec, 0, theta_R_m,
                                                                                                self.d, b_m,
                                                                                                order=2)  # source diffraction at mirror wedge
        D_m_down = self.Diffraction(k_vec, theta_S_m, self.n * np.pi, a_m, self.d) * self.Diffraction(k_vec, 0,
                                                                                                      theta_R_m, self.d,
                                                                                                      b_m,
                                                                                                      order=2)  # source reflection diffraction at mirror wedge
        TF = D_up + D_down + D_m_up + D_m_down
        return TF
    ...
    def TF_triangle(self, recorder_number):
        ...
        k_vec, _, _ = self.k_vec()
        theta_R = 2 * np.pi - phi - np.arctan([3 / 3, 5 / 3, 7 / 3])[
            recorder_number - 1]  # angle between recorder and sheet
        theta_R_m = 2 * np.pi - phi - np.arctan([3 / 5, 5 / 5, 7 / 5])[
            recorder_number - 1]  # angle between recorder and mirror sheet
        theta_S, theta_S_m = (
        np.arctan(15 / 19) - phi, np.arctan(15 / 21) - phi)  # angle between source and sheet and mirror source
        a, a_m = (np.sqrt(1.9 ** 2 + 1.5 ** 2) * self.d,
                  np.sqrt(2.1 ** 2 + 1.5 ** 2) * self.d)  # distance from source to top of sheet and mirror source
        b = np.array([np.sqrt(1.5 ** 2 + 1.5 ** 2) * self.d, np.sqrt(1.5 ** 2 + 2.5 ** 2) * self.d,
                      np.sqrt(1.5 ** 2 + 3.5 ** 2) * self.d])[
            recorder_number - 1]  # distance from recorder to top of sheet
        b_m = np.array([np.sqrt(2.5 ** 2 + 1.5 ** 2) * self.d, np.sqrt(2.5 ** 2 + 2.5 ** 2) * self.d,
                        np.sqrt(2.5 ** 2 + 3.5 ** 2) * self.d])[
            recorder_number - 1]  # distance from recorder to top of mirror sheet
        D_up = self.Diffraction(k_vec, theta_S, theta_R, a, b)  # source diffraction
        D_m_up = self.Diffraction(k_vec, theta_S_m, theta_R, a_m, b)  # source reflection diffraction
        D_down = self.Diffraction(k_vec, theta_S, theta_R_m, a, b_m)  # source diffraction at mirror wedge
    ...
    def Diffraction(self, k_vec, theta_S, theta_R, a, b, order=1):
        ...
        if self.obj == 'thick' and order == 1:
            ...
            C2D = k_vec * self.c * hankel2(0, k_vec * a) * np.exp(-1j * k_vec * b) / (
                        4 * np.sqrt(L))  # Propagation factor 2D
